...
<header>
    <div class="container">
        ...
        <div class="page-title" style="color:#009999;">
            <h1 class="display-4"><i>My notes</i></h1>
        </div>
    </div>

    <div class="universal-notes-actions">
        <form:form method="POST" modelAttribute="noteForm" action="${contextPath}/note-editor?mode=create" style='display:inline-block;'>
            <button class="btn btn-primary btn-lg" type="submit">Add a note</button>
        </form:form>

        <button type="button" class="btn btn-danger btn-lg" onclick="redirectToPublic()">View public notes</button>
        <button type="button" class="btn btn-warning btn-lg" onclick="redirectToArchive()">View my archived notes</button>
    </div>
</header>
<body>
  <div class="container">
    ...
    <c:choose>
        <c:when test="${not empty nonArchivedNotesList}">
            <div class="notes-list">
                <c:forEach items="${nonArchivedNotesList}" var="note" varStatus="notesLoop">
                    <div class="notes-list-note">
                        <h6><font size="+1">${note.title}</font></h6>
                        ...
                        <div class="notes-list-note-buttons">
                            ...
                            <c:if test="${note.published}">
                                <button type="button" class="btn btn-danger btn-sm" disabled>Published</button>
                            </c:if>
                        </div>
                    </div>
